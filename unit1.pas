...
begin
  ...
  if Round(_enemy.GetHealth()) > 0 then
    Memo_Description.Lines.AddText('The '+_enemy.GetName()+' has '+FloatToStr(Round(_enemy.GetHealth()))+' health left.')
  else
...
begin
  Memo_Description.Clear();
  Memo_Description.Lines.AddText(_boss.GetName());
  Memo_Description.Lines.Add('');
  if Round(_boss.GetHealth()) > 0 then
    Memo_Description.Lines.AddText('The '+_boss.GetName()+' has '+FloatToStr(Round(_boss.GetHealth()))+' health left.')
  else
    Memo_Description.Lines.AddText('The '+_boss.GetName()+' has '+' 0 health left.');
...
begin
  //Ebene 1
  begin
    CreateARoom('Your in your cell ...'+sLineBreak+'But you have a Bonfire!'+sLineBreak+sLineBreak+'Praise The Sun!', 'Images/Rooms_lvl1/Cell1.png', 2, 0, 0);
    CreateARoom('Erste Kreuzung.', 'Images/Rooms_lvl1/RoomAfterStartCell.png', 3, 0, 0);
    ...
    RoomArr[3, 0, 0].AddBoss(TBoss.Create('Test Boss', 'undefeated', 'Images/RoomObjects/Dealer.png', 1, 50, 50));

    CreateARoom('Der Raum mit der Ratte.', 'Images/Rooms_lvl1/MiddleCorridorClosedCells.png', 3, 1, 0);
    RoomArr[3, 1, 0].AddEnemy(TEnemy.Create('Rat', 20, 5, 'Images/Enemies/AAAAA.png'));
    RoomArr[3, 1, 0].EnemyArr[0].SetResistants(1, 1, 1);
    RoomArr[3, 1, 0].EnemyArr[0].SetItemDrop(TItem.Create('Literely just Trash', 'Like acually.', 'Images/Items/ITEM.png'));
    //RoomArr[2, 0, 0].EnemyArr[0].SetWeaponDrop(TWeapon.Create('Test Wep', 'Hi, i am a test wep.', 'Images/Items/ITEM.png', 1, 2, 3, 4));
    RoomArr[3, 1, 0].AddEnemy(TEnemy.Create('Rat', 20, 5, 'Images/Enemies/AAAAA.png'));

    CreateARoom('Hier Liegt eine Eisenstange', 'Images/Rooms_lvl1/Cell2.png', 4, 0, 0);
    RoomArr[4, 0, 0].AddWeapon(TWeapon.Create('Iron Bar', 'A brocken piece of a former cell.'+sLineBreak+'It is a bit rosty but can still function as a simple weapon.', 'Images/Items/IronBar.png', 15, 0, 0, 0));
    CreateARoom('Vier Wege von hier aus', 'Images/Rooms_lvl1/MiddleCorridorCross.png', 3, 2, 0);
    CreateARoom('Du siehst eine Waffe im nächsten Raum', 'Images/Rooms_lvl1/RoomBeforeDagger.png', 2, 2, 0);
    CreateARoom('WOW hier liegt tatsächlich ein Dolch', 'Images/Rooms_lvl1/RoomWithDagger.png', 1, 2, 0);
    RoomArr[1,2,0].AddWeapon(TWeapon.Create('Dagger', 'A small Dagger lack power or reach,'+sLineBreak+'but can deal quick consecutive hits due to their light weight', 'Images/Items/Dagger.png', 0, 8, 0, 0));
    CreateARoom('F*cking Goblins', 'Images/Rooms_lvl1/RoomWithGoblin.png', 3, 3, 0);
    RoomArr[3,3,0].SetDoorTop(true);
    RoomArr[3,3,0].SetDoorIndexTop(0);
    RoomArr[3, 3, 0].AddEnemy(TEnemy.Create('Goblin', 30, 10, 'Images/Enemies/AAAAA.png'));
    RoomArr[3, 3, 0].EnemyArr[0].SetResistants(0.7,1.3,1);
    RoomArr[3, 3, 0].EnemyArr[0].SetWeaponDrop(TWeapon.Create('A fancy Sword', 'With this Sword you can slash through hords of enemies.', 'Images/Items/Sword.png', 0, 0, 18, 0));
    CreateARoom('I see trouble', 'Images/Rooms_lvl1/RoomBeforeRats.png', 4, 2, 0);
    CreateARoom('And we make it tripple', 'Images/Rooms_lvl1/RoomWithRats.png', 5, 2, 0);
    RoomArr[5, 2, 0].AddEnemy(TEnemy.Create('Rat', 20, 5, 'Images/Enemies/AAAAA.png'));
    RoomArr[5, 2, 0].EnemyArr[0].SetResistants(1, 0.8, 1.2);
    RoomArr[5, 2, 0].AddEnemy(TEnemy.Create('Rat', 20, 5, 'Images/Enemies/AAAAA.png'));
    RoomArr[5, 2, 0].EnemyArr[1].SetResistants(1, 0.8, 1.2);
    RoomArr[5, 2, 0].AddEnemy(TEnemy.Create('Rat', 20, 5, 'Images/Enemies/AAAAA.png'));
    RoomArr[5, 2, 0].EnemyArr[2].SetResistants(1, 0.8, 1.2);
    CreateARoom('Praise the Goddess!', 'Images/Rooms_lvl1/RoomWithHealStature.png', 6, 2, 0);
    RoomArr[6, 2, 0].AddRoomObject(TRoomObject.Create('A Stature of an unknown Godess', '', 'Images/RoomObjects/StatureOfAnUnknownGod.png'));
    RoomArr[6, 2, 0].RoomObjectArr[0].SetHealing();
    //CreateARoom('Praise the Bonfire', 'Images/Rooms/Höle.png', 5, 3, 0);
    //RoomArr[5,3,0].SetBlockedLeft(true);
    CreateARoom('Leerer Raum oder so', 'Images/Rooms_lvl1/RoomAfterRatsAndBeforeGoblin.png', 5, 3, 0);
    //RoomArr[4,3,0].SetBlockedRight(true);
    CreateARoom('Drop den Schlüssel Goblin', 'Images/Rooms_lvl1/RoomWithGoblinWithKey.png', 5, 4, 0);
    RoomArr[5, 4, 0].AddEnemy(TEnemy.Create('AAAAA', 30, 10, 'Images/Enemies/AAAAA.png'));
    RoomArr[5, 4, 0].EnemyArr[0].SetResistants(0.7, 1.3, 1);
    RoomArr[5, 4, 0].EnemyArr[0].SetItemDrop(TItem.Create('alter Schlüssel', 'Dieser Schlüssel scheint zu einer alte Tür irgendwo in diesem Höhlensystem zu gehören', 'Images/Items/Key1.png', 0));
    CreateARoom('Useless ahead', 'Images/Rooms_lvl1/RoomAfterGoblinWithKey.png', 3, 4, 0);
    CreateARoom('Zum Glück hatte ich den schlüssel', 'Images/Rooms_lvl1/RoomBeforeBoss.png', 3, 4, 0);
    RoomArr[3,4,0].SetDoorBottom(true);
    RoomArr[3,4,0].SetDoorIndexBottom(0);
    CreateARoom('Estus vorraus', 'Images/Rooms_lvl1/RoomWithHealItem.png', 2, 4, 0);
    RoomArr[2,4,0].AddItem(TItem.Create('Heiltrank', 'Dieses Elexier stellt deine Lebenskraft wieder her', 'Images/Items/HealingItem.png'));
    CreateARoom('This is so sad. Alexa, play Gwyns theme', 'Images/Rooms/Höle.png', 2, 5, 0);
    RoomArr[3,5,0].AddRoomObject(TRoomObject.Create('Ladder','The height of this ladder is beyond comprehension','Images/Rooms_lvl1/BossRoomWithLadder.png'));
    RoomArr[3,5,0].RoomObjectArr[0].SetLadder();
  end;

  //Ebene 2
  begin
    CreateARoom('Bossraum.', 'Images/Rooms/Höle.png', 2, 6, 1);
    CreateARoom('Hier liegt ein Skill.', 'Images/Rooms/Höle.png', 0, 5, 1);
    CreateARoom('Drei Wege.', 'Images/Rooms/Höle.png', 1, 4, 1);
    CreateARoom('Hier gehts zum Boss.', 'Images/Rooms/Höle.png', 2, 5, 1);
    RoomArr[2,5,1].setblockedright(true);
    CreateARoom('Treppe zu Ebene 3.', 'Images/Rooms/Höle.png', 3, 5, 1);
    RoomArr[3,5,1].setblockedleft(true);
    RoomArr[3,5,1].setdoorright(true);
    RoomArr[3,5,1].setdoorindexright(1);
    CreateARoom('Startraum der zeiten Ebene.', 'Images/Rooms_lv2/part1/StartRoomWithLadder.png', 4, 5, 1);
    RoomArr[4,5,1].setdoorleft(true);
    RoomArr[4,5,1].setdoorindexleft(1);
    CreateARoom('Shiny Truhe.', 'Images/Rooms/Höle.png', 6, 5, 1);
    CreateARoom('Schatz vorraus?.', 'Images/Rooms/Höle.png', 1, 4, 1);
    RoomArr[1,4,1].setblockedright(true);
    CreateARoom('Gut, dass ich den Schlüssel hatte.', 'Images/Rooms/Höle.png', 2, 4, 1);
    RoomArr[2,4,1].setblockedleft(true);
    RoomArr[2,4,1].setdoorright(true);
    RoomArr[2,4,1].setdoorindexright(3);
    CreateARoom('Hätte ich doch nur Schlüssel.', 'Images/Rooms/Höle.png', 3, 4, 1);
    ...
    CreateARoom('F*cking Skelett.', 'Images/Rooms/Höle.png', 4, 4, 1);
    RoomArr[4,4,1].setblockedbottom(true);
    CreateARoom('Skelett im Doppelpack.', 'Images/Rooms/Höle.png', 5, 4, 1);
    RoomArr[5,4,1].setblockedbottom(true);
    CreateARoom('Schatz vorraus!.', 'Images/Rooms/Höle.png', 6, 4, 1);
    CreateARoom('Heal me up Scotty!.', 'Images/Rooms/Höle.png', 0, 3, 1);
    CreateARoom('Mimikrier mich nicht.', 'Images/Rooms/Höle.png', 1, 3, 1);
    ...
    CreateARoom('Zum Glück hatte ich noch einen Schlüssel.', 'Images/Rooms_lvl2/part1/RoomWithTwoDoors2.png', 3, 3, 1);
    ...
    CreateARoom('Diese Wand sieht nicht sehr stabil aus....', 'Images/Rooms/Höle.png', 4, 3, 1);
    RoomArr[4,3,1].setdoorleft(true);
    RoomArr[4,3,1].setdoorbottom(true);
    RoomArr[432,1].setblockedtop(true);
    CreateARoom('Random Wächter.', 'Images/Rooms/Höle.png', 5, 3, 1);
    RoomArr[5,3,1].setblockedtop(true);
    RoomArr[5,3,1].setblockedbottom(true);
    CreateARoom('Mehr Skills.', 'Images/Rooms/Höle.png', 6, 3, 1);
    CreateARoom('Noch ein Skelett.', 'Images/Rooms/Höle.png', 3, 2, 1);
    RoomArr[3,2,1].setblockedright(true);
    CreateARoom('Ein Agriffsboost liegt hinter dieser Wand.', 'Images/Rooms/Höle.png', 4, 2, 1);
    RoomArr[4,2,1].setblockedleft(true);
    CreateARoom('Hey, wanna buy some Keys.', 'Images/Rooms_lvl2/part1/RoomWithWardenAndDealer.png', 5, 2, 1);
    RoomArr[5,2,1].setblockedtop(true);
    CreateARoom('Dieser Wächter hat 100 pro einen Schlüssel.', 'Images/Rooms/Höle.png', 3, 1, 1);
  end;
  ...
  begin
    ...
    RoomArr[3, 5, 2].EnemyArr[0].SetResistants(1, 1, 1);
    ...
    RoomArr[1, 4, 2].EnemyArr[0].SetResistants(1, 1, 1);
    ...
    RoomArr[2, 4, 2].EnemyArr[0].SetResistants(1, 1, 1);
    RoomArr[2, 4, 2].AddEnemy(TEnemy.Create('Prediger', 20, 5, 'Images/Enemies/AAAAA.png'));
    RoomArr[2, 4, 2].EnemyArr[0].SetResistants(1, 1, 1);
    ...
    RoomArr[6, 4, 2].EnemyArr[0].SetResistants(1, 1, 1);
    ...
    RoomArr[5, 3, 2].EnemyArr[0].SetResistants(1, 1, 1);
    RoomArr[5, 3, 2].AddEnemy(TEnemy.Create('Prediger', 20, 5, 'Images/Enemies/AAAAA.png'));
    RoomArr[5, 3, 2].EnemyArr[0].SetResistants(1, 1, 1);
    ...
    RoomArr[3, 2, 2].EnemyArr[0].SetResistants(1, 1, 1);
